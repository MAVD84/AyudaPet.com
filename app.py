...
MASCOTAS_FILE = os.getenv("MASCOTAS_FILE", "mascotas.json")
app.config['MAX_CONTENT_LENGTH'] = int(os.getenv("MAX_CONTENT_LENGTH", 16 * 1024 * 1024))

# ─── LabsMobile config ────────────────────────────────────────────────────────
LABSMOBILE_USER   = os.getenv("LABSMOBILE_USER", "")
LABSMOBILE_TOKEN  = os.getenv("LABSMOBILE_TOKEN", "")
LABSMOBILE_SENDER = os.getenv("LABSMOBILE_SENDER") or None  # None si está vacío
...
def hash_password(password):
    """SHA-256 simple. Para producción considera bcrypt."""
...
def user_has_password(telefono):
    u = find_user(telefono)
    return u is not None and bool(u.get("password_hash"))

def set_user_password(telefono, password):
...
def guardar_otp(telefono, codigo):
    otp_store[telefono] = {"code": codigo, "expires": time.time() + 300}
...
.divider { display:flex; align-items:center; gap:10px; margin:18px 0; color:#94a3b8; font-size:.82em; }
...
def registro():
    ...
    <div class='auth-wrap'>
      <div class='auth-box'>
        <h1>🐾 Crear cuenta</h1>
        <p class='sub'>Ingresa tu número de WhatsApp para recibir un código de verificación (solo esta vez).</p>
...
def crear_password():
    ...
    if request.method == 'POST':
        ...
        if len(pwd1) < 6:
            ...
            set_user_password(telefono, pwd1)
    ...
    <div class='auth-wrap'>
      <div class='auth-box'>
        ...
        <p class='sub'>Esta será tu contraseña para iniciar sesión sin necesidad de recibir SMS cada vez.</p>
        {% if error %}<div class='alert alert-error'>{{ error }}</div>{% endif %}
        <form method='POST'>
          <div class='form-group'>
...
def login():
    ...
    if request.method == 'POST':
        ...
        if not telefono:
            ...
            # Si ya tiene contraseña, redirigir a login con contraseña
            if user_has_password(telefono):
                return redirect(url_for('login_password', tel=telefono))
            # Si no tiene contraseña aún, usar OTP (usuarios anteriores a este cambio)
            else:
                codigo = generar_otp()
                guardar_otp(telefono, codigo)
                ok, _ = enviar_sms_otp(telefono, codigo)
    ...
    <div class='auth-wrap'>
      <div class='auth-box'>
        ...
        <p class='sub'>Ingresa tu número de WhatsApp para continuar.</p>
        {% if error %}<div class='alert alert-error'>{{ error|safe }}</div>{% endif %}
        <form method='POST'>
          <div class='form-group'>
            <label>Número de WhatsApp (con código de país)</label>
...
def login_password():
    ...
    <div class='auth-wrap'>
      <div class='auth-box'>
        ...
        <p class='sub'>Número: <strong>{{ tel[:4] }}****{{ tel[-4:] }}</strong></p>
        {% if error %}<div class='alert alert-error'>{{ error }}</div>{% endif %}
        <form method='POST'>
          <div class='form-group'>
        ...
        <a href='/login/recuperar?tel={{ tel }}'>
          <button class='btn-secondary' type='button'>🔄 Olvidé mi contraseña (SMS)</button>
        </a>
...
# ══════════════════════════════════════════════════════════════════════════════
# LOGIN — recuperar contraseña via OTP
# ══════════════════════════════════════════════════════════════════════════════
@app.route('/login/recuperar')
def login_recuperar():
    telefono = request.args.get("tel","")
    if not telefono or not find_user(telefono):
        return redirect(url_for('login'))
    codigo = generar_otp()
    guardar_otp(telefono, codigo)
    ok, _ = enviar_sms_otp(telefono, codigo)
    if ok:
        return redirect(url_for('verificar_login_otp', tel=telefono, reset=1))
    return redirect(url_for('login_password', tel=telefono))

# ══════════════════════════════════════════════════════════════════════════════
# LOGIN — verificar OTP (para usuarios sin contraseña o recuperación)
# ══════════════════════════════════════════════════════════════════════════════
@app.route('/login/verificar', methods=['GET','POST'])
def verificar_login_otp():
    telefono = request.args.get("tel","")
    es_reset = request.args.get("reset","0") == "1"
    error = ""
    if not telefono:
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if ok:
            if es_reset:
                # Llevar a cambiar contraseña
                session["pending_password_phone"] = telefono
                return redirect(url_for('cambiar_password'))
            else:
                # Usuario sin contraseña aún: iniciar sesión y pedir que cree una
                session["telefono"] = telefono
                session["pending_password_phone"] = telefono
                return redirect(url_for('crear_password'))
        else:
